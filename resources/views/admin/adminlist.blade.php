            ...
            <table class="w-full text-center">
                <thead>
                    <tr class="text-gray-600 text-lg font-semibold">
                        ...
                        <th class="px-8 py-2 hidden md:table-cell">ROLE</th>
                ...
                <tbody>
                    ...
                    <tr class="text-gray-800 text-lg">
                        ...
                        <td class="px-4 py-3 hidden sm:table-cell">
                            <button data-modal-target="popup-modal-{{ $admin->id }}" data-modal-toggle="popup-modal-{{ $admin->id }}" class="block text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-800" type="button">
                                REMOVE ADMIN
                            </button>
                        
                            <!-- Modal (Dynamic ID) -->
                            <div id="popup-modal-{{ $admin->id }}" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                                        ...
                                        <div class="p-4 md:p-5 text-center">
                                            ...
                                            <div class="flex flex-col sm:flex-row justify-center items-center gap-4">
                                                <form action="{{ route('removeadmin', ['id' => $admin->id]) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button data-modal-hide="popup-modal-{{ $admin->id }}" type="submit"
                                                ...
                                                <button data-modal-hide="popup-modal-{{ $admin->id }}" type="button"
