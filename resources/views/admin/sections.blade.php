<x-admin-dashboard-layout>
            ...
            <div class="bg-gray-100 p-6 rounded-lg shadow-md">
                <form action="{{ route('createsection') }}" method="POST" class="w-full max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" id="name" name="name" placeholder="e.g. Oxygen" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 mt-2"/>
                    </div>
                    <div class="mb-4">
                ...
                <table class="min-w-full table-auto text-left">
                    <thead class="bg-gray-100 text-md text-gray-600 text-center">
                        <tr>
                            ...
                            <th class="px-4 py-2">EDIT MAX LEARNERS</th>
    ...
    <button data-modal-target="edit-max-learner-modal-{{ $section->id }}" data-modal-toggle="edit-max-learner-modal-{{ $section->id }}" 
        class="text-white bg-blue-600 hover:bg-blue-700 focus:ring-4 focus:outline-none focus:ring-blue-300 font-bold rounded-lg text-sm px-5 py-2.5 text-center">
        EDIT MAX LEARNERS
    </button>

    <div id="edit-max-learner-modal-{{ $section->id }}" tabindex="-1" 
        ...
        <div class="relative p-4 w-full max-w-md max-h-full">
            <div class="relative bg-white rounded-lg shadow dark:bg-white-700">
                <button type="button" 
                    ...
                    data-modal-hide="edit-max-learner-modal-{{ $section->id }}">
                ...
                <div class="p-6">
                    <h3 class="text-lg font-bold mb-4 text-gray-800">Edit Max Learners for {{ $section->name }}</h3>
                    <form action="{{ route('sections.updateMaxLearnerSingle', $section->id) }}" method="POST">
                        @csrf
                        @method('PUT')

                        <div class="mb-4">
                            <label for="max_learner-{{ $section->id }}" class="block mb-2 text-sm font-medium text-gray-700">Max Learners</label>
                            <input 
                                type="number" 
                                name="max_learner" 
                                id="max_learner-{{ $section->id }}" 
                                value="{{ $section->max_learner }}" 
                                min="1" 
